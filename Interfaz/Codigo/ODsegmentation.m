function [OD, maskOD, bordesOD, Validez]=ODsegmentation(RGB, cuadrado)  %RGB tiene que ser ROI_SinCaps
        R=uint8(RGB(:,:,1));
    LAB=rgb2lab(RGB);
    L=LAB(:,:,1);
    L=round(L);
    ...
    % if (NumOfObjects ~= 1) || (NumOfObjects == 1 && Area < 125000)
    %     [bordesOD, maskOD, OD, NumOfObjects, objects, Area]=boundariesOD(R, RGB, 20, 600, 0.1, 50, sizeClosing, sizeOpening);
    %     if (NumOfObjects == 1 && Area > 125000) && (NumOfObjects == 1 && objects.Circularity < 0.95)
    %         [bordesOD, maskOD,OD]=corregirCircularidadBaja(bordesOD,RGB);
    %     end
    % end

    %Si el �rera es menor a 50000 no cuenta (se ha cogido ese valor porque si o
    %si el OD va a ser mayor que eso, teniendo en cuenta que de normal tienen
    %200000). Al final he puesto 125000 que va mejor.

    % if Rmal==1%Pon Rmal=1 o Rmal=2 en algun lado para que salga en al foto y lo puedas identificar
    %     bordesOD = imoverlay(bordesOD,cuadrado2,'c');
    %     maskOD = imoverlay(maskOD,cuadrado2,'c');
    %     OD = imoverlay(OD,cuadrado2,'c');
    % elseif Rmal==2
    %     bordesOD = imoverlay(bordesOD,cuadrado2,'m');
    %     maskOD = imoverlay(maskOD,cuadrado2,'m');
    %     OD = imoverlay(OD,cuadrado2,'m');
    % end

    if NumOfObjects ~= 1
        bordesOD = imoverlay(bordesOD,cuadrado,'r');
        maskOD = imoverlay(maskOD,cuadrado,'r');
        OD = imoverlay(OD,cuadrado,'r');
    elseif (NumOfObjects == 1 && Area < 125000)
        bordesOD = imoverlay(bordesOD,cuadrado,'y');
        maskOD = imoverlay(maskOD,cuadrado,'y');
        OD = imoverlay(OD,cuadrado,'y');
    elseif (NumOfObjects == 1 && objects.Circularity < 0.8)
        bordesOD = imoverlay(bordesOD,cuadrado,'b');
        maskOD = imoverlay(maskOD,cuadrado,'b');
        OD = imoverlay(OD,cuadrado,'b');
    elseif (NumOfObjects == 1 && objects.Area > 640000)
         bordesOD = imoverlay(bordesOD,cuadrado,'w');
         maskOD = imoverlay(maskOD,cuadrado,'w');
         OD = imoverlay(OD,cuadrado,'w');
    end
